<?php
  ...
  if(isset($headers['X-Api-Key'])){ // Primer nivel de seguridad
    if($headers['X-Api-Key'] === $api_key){ // Segundo nivel de seguridad
      ...
      $query = "SELECT * FROM cursos"; // Filtra a todos los maestros de los han sido etiquetados como eliminados.
      ...
      while ( $r = $select_all->fetch_assoc() ) {
        $cursos[] = $r; // #TODO: Verificar que datos no vamos a necesitar en está consulta 
      }
      $mysqli->close();
