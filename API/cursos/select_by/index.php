    ...
    if($headers['X-Api-Key'] === $api_key){ // Segundo nivel de seguridad
      ...
      $query = "SELECT 
                * 
                FROM cursos 
                WHERE deleted != 1
                AND id_curso = '$curso->id_curso'"; // Retorna la información del curso solicitado.
      ...
      while ( $r = $select_by->fetch_assoc() ) {
        $curso_solicitado[] = $r; // #TODO: Verificar que datos no vamos a necesitar en está consulta 
      }
      $mysqli->close();
      ...
      $respuesta = [
        ...
        'msj' => 'Ya se encuentra disponible la información del curso solicitado'
      ];
      exit( json_encode( $respuesta ) );
